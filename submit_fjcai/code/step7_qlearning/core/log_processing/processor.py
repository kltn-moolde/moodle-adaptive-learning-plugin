...
class StudentContext:
    """
    Simple student context tracker for batch log processing
    Tracks student state as logs are processed chronologically
    """
    def __init__(self, student_id: int, cluster_id: int):
        self.student_id = student_id
        self.cluster_id = cluster_id
        self.current_module = 0
        self.progress = 0.0
        self.avg_score = 0.0
        self.recent_action = 'unknown'
        self.is_stuck = False
        self.log_count = 0
        self.grade_count = 0
    
    def update_from_log_entry(self, log_dict: dict):
        """Update context from a log entry"""
        self.log_count += 1
        self.current_module = log_dict.get('contextinstanceid', self.current_module)
        
        # Map action type
        eventname = log_dict.get('eventname', '')
        component = log_dict.get('component', '')
        action = log_dict.get('action', '')
        self.recent_action = self._map_action_type(eventname, component, action)
        
        # Update progress (simplified - assume each log entry increases progress slightly)
        if self.log_count > 0:
            self.progress = min(1.0, self.log_count * 0.01)  # Simplified progress tracking
    
    def update_from_grade_entry(self, grade_dict: dict):
        """Update context from a grade entry"""
        self.grade_count += 1
        grade = grade_dict.get('finalgrade', 0.0)
        
        # Update average score
        if self.grade_count == 1:
            self.avg_score = grade
        else:
            # Simple moving average
            self.avg_score = (self.avg_score * (self.grade_count - 1) + grade) / self.grade_count
    ...
    def _map_action_type(self, eventname: str, component: str, action: str) -> str:
        """Map Moodle log event to action type"""
        # Simplified mapping
        if 'viewed' in eventname.lower() or 'view' in action.lower():
            return 'view'
        elif 'submitted' in eventname.lower() or 'submit' in action.lower():
            return 'submit'
        elif 'completed' in eventname.lower() or 'complete' in action.lower():
            return 'complete'
        elif 'attempted' in eventname.lower() or 'attempt' in action.lower():
            return 'attempt'
        else:
            return 'unknown'
