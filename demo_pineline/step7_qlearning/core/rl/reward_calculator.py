...
Key Improvements:
- Supports 6D state from StateBuilderV2 (cluster, module, progress, score, phase, engagement)
...
class RewardCalculatorV2:
    ...
    State Tuple (6D from StateBuilderV2):
    =====================================
    (cluster_id, module_idx, progress_bin, score_bin, learning_phase, engagement_level)
    ...
    def __init__(
        ...
        if reward_config_path is None:
            # Go up 3 levels from core/rl/reward_calculator.py to project root
            reward_config_path = Path(__file__).parent.parent.parent / 'config' / 'reward_config.json'
        self.config = self._load_config(Path(reward_config_path))
        ...
        if course_id is not None:
            ...
            try:
                ...
                if po_lo_path is None:
                    po_lo_path = Path(__file__).parent.parent.parent / 'data' / 'Po_Lo.json'
                else:
                    # Convert to Path and resolve relative paths from project root
                    po_lo_path = Path(po_lo_path)
                    if not po_lo_path.is_absolute():
                        po_lo_path = Path(__file__).parent.parent.parent / po_lo_path
                if midterm_weights_path is None:
                    midterm_weights_path = Path(__file__).parent.parent.parent / 'data' / 'midterm_lo_weights.json'
                else:
                    # Convert to Path and resolve relative paths from project root
                    midterm_weights_path = Path(midterm_weights_path)
                    if not midterm_weights_path.is_absolute():
                        midterm_weights_path = Path(__file__).parent.parent.parent / midterm_weights_path
    ...
    def calculate_reward(
        ...
        Args:
            state: Current 6D state tuple from StateBuilderV2
            ...
            previous_state: Previous 6D state tuple (for delta calculation)
        ...
        """
        # Unpack 6D state
        cluster_id, module_idx, progress_bin, score_bin, learning_phase, engagement_level = state
        ...
        # ===================================================================
        # Component 8: Frustration Penalty (removed - no longer in state)
        # ===================================================================
        # Frustration level removed from 6D state format
    ...
    def calculate_reward_with_breakdown(
        ...
        # ===================================================================
        # Component 9: Stuck Penalty (removed - no longer in state)
        # ===================================================================
        # Frustration level removed from 6D state format
...
def test_reward_calculator():
    """Test RewardCalculatorV2 with 6D state support + LO mastery integration"""
    print("=" * 80)
    print("Testing RewardCalculatorV2 - Refactored (6D State + LO Mastery)")
    print("=" * 80)
