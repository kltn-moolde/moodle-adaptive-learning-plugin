...
class RewardCalculatorV2:
    ...
    def calculate_reward_with_breakdown(
        ...
        # ===================================================================
        phase_cfg = self.config['reward_components']['learning_phase']
        if learning_phase in phase_cfg.get('bonus_phases', []):
            phase_reward = phase_cfg.get('bonus', 1.0)
            reward += phase_reward
        ...
        # ===================================================================
        engage_cfg = self.config['reward_components']['engagement']
        if engagement_level >= engage_cfg.get('threshold', 2):
            engage_reward = engage_cfg.get('bonus', 1.0)
