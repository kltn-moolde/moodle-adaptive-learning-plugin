...
# -*- coding: utf-8 -*-
"""
State Builder V2 - Redesigned state representation with 7 dimensions
"""
...
class StateBuilderV2:
    """State Builder for Q-Learning System V2 with 7D state"""
    ...
    # Frustration levels (TEMPORARILY DISABLED - always return 0)
    # Future enhancement: re-enable multi-level frustration detection
    # FRUSTRATION_LEVELS = {
    #     0: "low",
    #     1: "medium", 
    #     2: "high"
    ...
    def calculate_state_space_size(self) -> int:
        """Calculate total state space size: clusters × modules × progress × score × phase × engagement × frustration"""
        # State space: 5 × 6 × 4 × 4 × 3 × 3 × 1 = 4,320 states
        # Reduced from 17,280 by: engagement 4→3 levels, frustration 3→1 level
        return self.n_clusters * self.n_modules * 4 * 4 * 3 * 3 * 1
    ...
    def convert_7d_to_6d(self, state_7d: Tuple) -> Tuple:
        ...
        Returns:
            6D state (cluster, module, progress, score, action_id, stuck)
        """
        if len(state_7d) != 7:
