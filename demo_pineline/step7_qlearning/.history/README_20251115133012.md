...
- **State 6D/7D**: Hỗ trợ cả 2 formats với auto-conversion (6D cho Q-table cũ, 7D cho training mới)
...
| **State Dimensions** | ~10+ | 7 | Đơn giản hơn |
| **LO Integration** | ❌ | ✅ | Mới |
...
# Get recommendations với LO mastery (optional)
...
**7D Format** (StateBuilderV2 - cho training mới):
```python
state_7d = (cluster_id, module_idx, progress_bin, score_bin, 
            learning_phase, engagement_level, frustration_level)
```

**6D Format** (Q-table best model - cho inference):
```python
state_6d = (cluster_id, module_idx, progress_bin, score_bin, 
            action_id, is_stuck)
```

**Auto-conversion**: API tự động convert 7D → 6D khi dùng Q-table cũ

| Dimension | 7D Values | 6D Values | Description |
|-----------|-----------|-----------|-------------|
| **cluster_id** | 0-4 | 0-4 | Student cluster (exclude teacher) |
| **module_idx** | 0-5 | 0-5 | Current module index |
| **progress_bin** | 0.25, 0.5, 0.75, 1.0 | 0.25, 0.5, 0.75, 1.0 | Quartile binned progress |
| **score_bin** | 0.25, 0.5, 0.75, 1.0 | 0.25, 0.5, 0.75, 1.0 | Quartile binned score |
| **learning_phase** | 0, 1, 2 | - | pre/active/reflective |
| **engagement_level** | 0, 1, 2 | - | low/medium/high |
| **frustration_level** | 0 | - | Currently disabled |
| **action_id** | - | 0-5 | Recent action type ID |
| **is_stuck** | - | True/False | Stuck status |
