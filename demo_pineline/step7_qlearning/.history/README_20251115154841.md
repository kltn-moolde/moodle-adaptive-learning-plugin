...
**Note**: State được build từ `features` với các trường:
- `cluster_id`: Cluster ID của học sinh
- `current_module_id`: Module ID từ course structure (sẽ được map thành `module_idx`)
- `module_progress`: Tiến độ module (0-1, sẽ được bin thành `progress_bin`)
- `avg_score`: Điểm trung bình (0-1, sẽ được bin thành `score_bin`)
- `recent_action_type`: Loại hành động gần nhất (0-5, dùng để tính `learning_phase` và `engagement_level`)

Các trường khác trong `features` (như `is_stuck`, `quiz_attempts`, etc.) là optional và không được sử dụng trong state building.
