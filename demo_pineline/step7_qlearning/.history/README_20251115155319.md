...
**Note**: State được build từ `features` với các trường bắt buộc:
- `cluster_id` (int): Cluster ID của học sinh (0-4, sau khi map và exclude teacher cluster)
- `current_module_id` (int): Module ID từ course structure (sẽ được map thành `module_idx` 0-5)
- `module_progress` (float): Tiến độ module (0-1, sẽ được bin thành `progress_bin`: 0.25, 0.5, 0.75, 1.0)
- `avg_score` (float): Điểm trung bình (0-1, sẽ được bin thành `score_bin`: 0.25, 0.5, 0.75, 1.0)
- `recent_action_type` (int, optional, default=0): Loại hành động gần nhất (0-5), được map thành action name và dùng để tính:
  - `learning_phase`: 0=pre-learning, 1=active-learning, 2=reflective-learning
  - `engagement_level`: 0=low, 1=medium, 2=high (dựa trên action weights và consistency)
...
Các trường khác trong `features` (như `is_stuck`, `quiz_attempts`, `quiz_failures`, `time_on_module`) là optional và không được sử dụng trong state building.
