...
class RecommendationService:
    ...
    def get_recommendations(
        ...
        # Convert indices to module IDs for Q-table lookup
        available_module_ids = []
        for idx in available_indices:
            action = self.action_space.get_action_by_index(idx)
            if action:
                available_module_ids.append(action.id)  # Use Moodle module ID
        
        # Get recommendations from agent (using module IDs)
        try:
            recommendations = self.agent.recommend_action(
                state=state,
                available_actions=available_module_ids,  # Pass module IDs, not indices
                top_k=top_k,
        ...
        # Note: recommendations now contain (module_id, q_value)
        results = []
        for module_id, q_value in recommendations:
            # Get action by module ID (not index)
            action = self.action_space.get_action_by_id(module_id)
            
            if action:
                # Find action index for consistent API response
                action_index = None
                for idx in range(self.action_space.get_action_count()):
                    if self.action_space.get_action_by_index(idx).id == module_id:
                        action_index = idx
                        break
                
                results.append({
                    'action_id': action_index if action_index is not None else module_id,
                    'module_id': module_id,  # Include module ID for reference
