...
class TrainingVisualizer:
    ...
    def __init__(
        ...
        Args:
            qtable_path: Path to trained Q-table (if None, uses models/qtable_{course_id}.pkl)
            course_id: Course ID for multi-course support (default: 5)
            output_dir: Directory to save plots
        """
        if qtable_path is None:
            import config
            qtable_path = str(config.get_model_path(course_id))
        self.qtable_path = qtable_path
        self.course_id = course_id  # Store course_id for later use
    ...
    def simulate_student_trajectory(
        ...
        for step in range(n_steps):
            ...
            # Map module_idx to lesson_id (for visualization, use module_idx as lesson_id)
            lesson_id = student.module_idx  # In visualization, module_idx can be used as lesson_id
            recommendation = self.recommender.recommend_activity(
                action=action,
                course_id=self.course_id,
                lesson_id=lesson_id,
                past_lesson_ids=None,  # Not tracked in visualization simulation
                future_lesson_ids=None,  # Not tracked in visualization simulation
...
def demo_visualizer(course_id: int = 5):
    ...
    import config
    qtable_path = str(config.get_model_path(course_id))
    viz = TrainingVisualizer(qtable_path=qtable_path, course_id=course_id)
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Visualize Q-Learning training results')
    parser.add_argument('--course-id', type=int, default=5, help='Course ID (default: 5)')
    args = parser.parse_args()
    demo_visualizer(course_id=args.course_id)
